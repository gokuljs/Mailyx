...
const BannerContent = ({ heading, credits, total, subtext }: Props) => {
  return (
    <div className="z-40">
      <div className="flex items-center gap-2 bg-gradient-to-r from-stone-300 to-stone-950 bg-clip-text text-xl font-semibold text-transparent">
        {heading}
      </div>
      {total && credits && total >= 0 && credits >= 0 && (
        <div className="mt-1 flex gap-1 text-sm text-zinc-300">
          <span className="text-stone-400">{credits}</span>/<span>{total}</span>{" "}
